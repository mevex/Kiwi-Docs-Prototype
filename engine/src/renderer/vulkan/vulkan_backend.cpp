...
b8 VulkanRenderer::Initialize(const char *ApplicationName, u32 Width, u32 Height)
{
	...
	RegenerateFramebuffers(&Context.Swapchain, Context.MainRenderPass.Handle);
...
void VulkanRenderer::RegenerateFramebuffers(VulkanSwapchain *Swapchain, VkRenderPass RenderPassHandle)
{
	for (u32 Idx = 0; Idx < Swapchain->ImageCount; ++Idx)
	{
		...
		Swapchain->Framebuffers[Idx].Create(RenderPassHandle, Context.FramebufferWidth, Context.FramebufferHeight,
...
b8 VulkanRenderer::RecreateSwapchain()
{
	...
	Context.Swapchain.Recreate(Context.FramebufferWidth, Context.FramebufferHeight);
