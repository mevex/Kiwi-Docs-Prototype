...
These batch files can be located in the Visual Studio installation folder, under `VC\Auxiliary\Build` (e.g. `C:\Program Files\Microsoft Visual Studio \20**\VC\Auxiliary\Build`). Run `vcvars64.bat` to configure the environment for building an x64 version of the project.
...
## Batch Structure
Running `build-all.bat` triggers a chain reaction that builds the entire project. It first builds `Engine.lib` (and `.dll`) first, then `Testbed.exe`, which dynamically links against the engine.
...
- **`/MTd`:** Statically links the CRT inside the binary. This is done to allow the binary to be standalone and not depend on any redistributable. Specifying `d` make the compiler use the debug version of the runtime library.
...
- **`/nologo`:** Suppress the display of the copyright banner. Same as compiler.
...
### Warnings
The two main flags used here are `/W4`, which sets the warning level to 4 (roughly equivalent to GCC’s `-Wall`) and `/WX`, applied to both the compiler and linker, that threats all warnings as errors.
...
- **`KIWI_ENGINE_EXPORTS`:** Used to resolve the `KIWI_API` macro to `__declspec(dllexport)` or `__declspec(dllimport)`. `KIWI_API` is applied to classes and function that are part of the game facing engine API. This macro is defined only when building the engine.
